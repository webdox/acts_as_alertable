module ActsAsAlertable
  class Alert < ActiveRecord::Base
	...
	def observable_dates
		return (alertables.map{|e| e.try(observable)}.map{|e| e.try(:to_date)}.uniq - [nil]) if alertable_model.attribute_names.include?(observable)
	...
	def alertables_for_date date
		...
		ids = alertables.select{|a| date.send(advanced_type_operator, a.send(observable).try(:to_datetime))}.map{|a| a.id } if kind == 'advanced_periodic'
